...
class HangmanGame:
    ...
    def play(self):
        while not self.end_of_game:
            ...
            if user_guess in self.used_letters:
                print(
                    f"You've already used that letter!\n See {self.used_letters}")
            elif user_guess not in self.chosen_word:

                print(
                    f"Your guess {user_guess} is not in the word. You lose a life.")
                self.lives -= 1
                if self.lives < 0:
                    self.lives = 0
    ...
    def play_again(self):
        self.lives = 6
        self.chosen_word = random.choice(difficulty.select_difficulty())
        self.display = []
